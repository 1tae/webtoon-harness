...
| 상황 | 에스컬레이션 대상 |
|------|-------------------|
| 0바이트/손상 3회 연속 실패 | art-director + 오케스트레이터 |
| codex 세션 한도 초과 지속 | 오케스트레이터 (동시 수 조정 필요) |
| 레퍼런스 시트 미확정 상태에서 렌더 요청 | art-director에 확정 요청. 확정 전 렌더 시작 금지 |
| ACCEPT-FLAG 패널 5개 이상 | 오케스트레이터 + quality-reviewer (품질 리스크 경고) |
| prompts.md 누락/불완전 | prompt-smith에 요청. 프롬프트 없이 임의 렌더 금지 |
| 전체 배치 렌더 실패 (네트워크 등) | 오케스트레이터에 즉시 에스컬레이션 |
