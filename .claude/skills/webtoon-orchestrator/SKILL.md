...
| Phase 전체 실패 (한 단계의 모든 에이전트 무응답/오류) | 해당 Phase를 중단하고 사용자에게 상태를 보고. 부분 산출물을 보존해 재시도 시 활용 |
| 에이전트 요청 후 응답 없음 | 1회 재요청 후에도 무응답이면 대체 에이전트 생성 또는 사용자에 에스컬레이션. 조용히 대기하지 않는다 |
| 산출물 파일 경로 불일치 | 즉시 교정하고 하류 에이전트에 경로 변경을 통지. 경로 불일치는 파이프라인 전체를 깨뜨린다 |
