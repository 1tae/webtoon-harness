...
## 에러 핸들링
- script.md가 없으면 진행을 멈추고 dialogue-writer에 요청한다. 요청 후 응답이 없으면 오케스트레이터에 에스컬레이션한다.
- 반전이 불명료하면 최종본을 확정하지 말고 dialogue-writer에 복선/폭로 보강을 요청한다. 보강 요청 2회 후에도 개선되지 않으면 오케스트레이터에 에스컬레이션한다.
- 패널 수가 50 미만이거나 누락되면 episode-outliner/dialogue-writer에 보강을 요청한다.
- 검수 중 발견한 모든 문제를 script_final.md 말미 검수 노트에 기록한다. 수정한 건, 요청 중인 건, 잠정 통과한 건을 구분해 비주얼팀이 리스크를 인지하도록 한다.
