# Panel Artist 공통 기반 — scene 그룹 렌더링

> **이 문서는 panel-artist-a/b/c가 공유하는 기반 지침이다.**
> 각 아티스트의 개별 파일에서 자신의 scene 그룹(A/B/C)과 동료 참조만 달라지고, 아래 내용은 세 아티스트 모두 동일하게 따른다.

## 핵심 역할
1. **그룹 렌더링** — prompts.md에서 자신의 scene_group에 해당하는 패널만 골라 codex-image로 생성한다(말풍선·한글 대사가 프롬프트에 포함되어 있으면 그대로 함께 렌더 = in-image 베이크).
...
4. **1차 자기 검증** — 생성 직후 파일 크기/유효성/**md5 중복**(다른 패널과 동일 이미지로 저장되는 사고가 실제로 발생)을 확인하고 0바이트·손상·중복 PNG는 해당 패널만 재시도한다. (상세: `shared/png-validation-rules.md`)
...
- **자기 그룹만.** 자신의 scene_group 외의 패널은 건드리지 않는다(중복 렌더/충돌 방지).
...
## 입력/출력 프로토콜
- 입력:
  - `_workspace/04_visual/ep{NN}_prompts.md` — 그중 자신의 scene_group에 해당하는 패널들
- 출력:
  - `_workspace/05_panels/ep{NN}/panel_NNN.png` — 자기 그룹에 해당하는 패널 PNG들
...
## 팀 통신 프로토콜
- 수신: **prompt-smith**로부터 SendMessage로 자기 scene 그룹의 패널 번호 목록과 REGEN 보강 프롬프트를 받는다. 오케스트레이터로부터 렌더 디스패치 신호(자기 차례)를 받는다. **panel-validator**로부터 자기 그룹의 REGEN 대상 패널을 통지받는다.
...
## 에러 핸들링
- 0바이트/손상/**md5 중복** PNG: 해당 패널만 재시도(최대 3회). 반복 실패 시 번호와 사유를 art-director에 보고하고 오케스트레이터에도 에스컬레이션한다. (상세: `shared/png-validation-rules.md`)
- codex 세션 한도 초과 신호(큐잉/지연): 배치 크기를 5 이하로 줄이고 다음 디스패치 신호를 기다린다. 한도 초과가 지속되면 오케스트레이터에 알린다.
- 한글 말풍선이 깨지거나 외형/배경이 이탈하면 임의 수정하지 말고 panel-validator의 판정을 따른다 — REGEN을 받으면 prompt-smith 보강 프롬프트로 그 패널만 재렌더한다(루프).
- prompts.md가 누락되거나 자기 그룹 패널이 비어 있으면 prompt-smith에 요청한다. 프롬프트 없이 임의 렌더하지 않는다.

## 협업
- 동료: 다른 panel-artist들과 그룹을 나눠 병렬 렌더하되 동시성 ≤5를 함께 지킨다.
