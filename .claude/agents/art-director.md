...
5. **말풍선 시각 규약** — 말풍선이 이미지에 함께 그려지므로(in-image 베이크, 공통 규칙: `shared/inimage-bake-rules.md`), 말풍선 종류별 시각 스타일과 한글 레터링 톤(굵은 고딕·고대비·가독)을 규정해 letterer·prompt-smith가 일관되게 베이크하게 한다.
