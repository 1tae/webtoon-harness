...
4. **샷 설계** — 패널마다 size/camera/composition/emotion/motion/dialogue_ref를 명시해 prompt-smith와 letterer가 곧바로 작업할 수 있게 한다.
...
- style-bible이 아직 없으면 장르 일반 화면비로 잠정 진행하되 art-director 확정 후 재점검 플래그를 남긴다. 잠정 진행 사실을 prompt-smith·letterer에 알린다.
- script_final.md가 없으면 진행을 멈추고 script-editor에 요청한다. 요청 후 응답이 없으면 오케스트레이터에 에스컬레이션한다.
- 대본의 패널 구조가 애매하거나 반전이 약하면 script-editor에 명확화를 요청한다. 임의로 해석해 샷리스트를 만들지 않는다 — 반전 타이밍이 어긋나면 회차 전체가 무너진다.
