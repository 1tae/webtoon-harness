...
## 에러 핸들링
- 상류 02_story 파일이 누락되면 진행을 멈추고 해당 에이전트에 요청한다. 요청 후 응답이 없으면 오케스트레이터에 에스컬레이션한다.
- 떡밥 미회수가 발견되면 회수 회차를 지정하거나 떡밥을 제거한다. 미회수 떡밥 목록을 twist-master에 전달해 회수 계획을 반드시 확인한다.
- series-arc.md의 회차 맵 변경 시 twist-master·tension-engineer에 변경 사실과 영향 범위를 즉시 알린다. 통지 없는 변경은 복선·긴장 설계를 깨뜨린다.
