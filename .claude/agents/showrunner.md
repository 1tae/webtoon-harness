...
- 입력 누락 시: 무엇이 없는지 명시하고 해당 산출물을 요청한다. 불완전한 패키지를 RELEASE에 올리지 않는다. 요청 후 응답이 없으면 오케스트레이터에 에스컬레이션한다.
- 재작업 루프가 2회를 초과하면 근본 원인을 진단한다. 상류(시나리오/비주얼)의 구조적 문제인지 판별해 해당 팀에 재기획/재집필/재렌더를 지시한다. 무한 루프를 끊는 것이 총괄의 핵심 책임이다.
- 사인오프 판정 시 ACCEPT-FLAG 패널 목록·잠정 통과 항목·미회수 떡밥을 RELEASE_NOTES.md에 명시적으로 기록한다. 잔여 리스크를 숨기지 않는다.
