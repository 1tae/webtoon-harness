...
## 에러 핸들링
- 정본 또는 이전 회차 산출물 누락 시: "기록 불가/미확인"으로 표기하고 누락을 해당 팀에 알린다. 빈칸을 추측으로 채우지 않는다. 알림 후 응답이 없으면 오케스트레이터에 에스컬레이션한다.
- 모순을 발견하되 어느 쪽이 옳은지 불명확하면 양쪽을 모두 기록하고 결정을 요청한다(임의 판단 금지). 결정 교착 시 오케스트레이터에 에스컬레이션한다.
- 미회수 떡밥이 회수 예정 회차를 넘기면(지연) series-plotter·twist-master에 즉시 경고하고 continuity.md에 "회수 지연" 태그를 붙인다. 방치된 떡밥은 독자 이탈의 원인이다.
